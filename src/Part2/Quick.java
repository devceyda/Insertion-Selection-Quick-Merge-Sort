...
public class Quick {
    ...
    private static boolean lessModified(Comparable v, Comparable w)
    //
    ...
    private static int partitionModified(Comparable[] a, int lo, int hi)
    //
    ...
    { // Partition into a[lo..i-1], a[i], a[i+1..hi].
        ...
        while (true) { // Scan right, scan left, check for scan complete, and exchange.
            while (lessModified(a[++i], v))
                if (i == hi)
                    break;
            while (lessModified(v, a[--j]))
                if (j == lo)
                    break;
            if (i >= j)
                break;
    ...
    public static void sortModified(Comparable[] a)
    //
    ...
    //
    {
        ...
        sortModified(a, 0, a.length - 1);
    }

    private static void sortModified(Comparable[] a, int lo, int hi)
    //
    ...
    //
    {
        if (hi <= lo)
            return;
        int j = partitionModified(a, lo, hi); // Partition (see page 291).
        sortModified(a, lo, j - 1); // Sort left part a[lo .. j-1].
        sortModified(a, j + 1, hi); // Sort right part a[j+1 .. hi].
    }
